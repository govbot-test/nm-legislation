...
Formatted, versioned legislative data for New Mexico. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for New Mexico — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/nm-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`nm-legislation`) — reads the scraper repo's output and:
...
To change New Mexico's configuration (branch pinning, runner, enabled jobs, labels), edit its entry in [`chn-openstates-files.yml`](https://github.com/chihacknight/govbot/blob/main/actions/pipeline-manager/chn-openstates-files.yml) and re-run `apply.py` — don't hand-edit the workflow files in this repo directly, since the next `apply.py` run will overwrite them.

Current settings for New Mexico:

- **State code**: `nm`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/nm-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:nm/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:nm/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:nm/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:nm/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/nm-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
